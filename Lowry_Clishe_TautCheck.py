...
#If a it's not a tautology, an espresso file for a witness cube will be placed in the folder Lowry_Clishe_Tautology_Witnesses
#Make sure this folder exists before running the script

#Test       1, 0.056    2, 0.432    3, 125.7    4, 36.74    5, 0.009    6, 0.268    7, 8.445    8, 0.051
#Bench      1, 0.004    2, 0.291    3, 0.547    4, 0.025    5, 0.018    6, 0.087    7, 0.031    8, 0.003    9, 2.196    10, 39.75    11, 105.9
...
#Originally based on Lowry_Clishe2_tautcheck then used sugestions from Gemini3_tautcheck to convert it to using the 
#'0' = 1, '1' = 2, '-' = 3 numpy method. The proper bit packing method seemed now well suited for GUR tautology checking since
#the whole unate paradigm focuses on moving values around more than it does executing math on them
#This was done instead of just starting with Gemini3_tautcheck because it was often much slower than Lowry_Clishe2_tautcheck
#Optimized with whatever I could find online, and a whole bunch of puting code smipets into AI to ask how to make it faster
#Often the AI would try to use numpy more heavilly but often would make things slower than a pythonic implementation
#This is the best ballance I could come up with.

#First it imports the espresso file as an uint8 numpy array with '0' = 1, '1' = 2, '-' = 3
#It then checks for special cases:
#1. If there are no dashes in the cover, count the number of unique cubes. It is a tautology if and only if there are 2^n
#   If it's not, the witness is the first cube it finds that isn't in the set
#2. If there is a universal cube of all dashes, the cover is a tautology
#3. Check if any pure unate (all 1 or all 0) columns exist. If so, the cover is not a tautology. The witness would be anything where the 
#   variable asociated with pure unate column is the oposite of the column value. To make it simple, for negative unate, the witness is all 0's, and positive is all 1's
#   Since this step is the first to deal with unate and binate columns, it takes a while since it has to generate lists of which
#   columns have '-' '1' or '0', and which are bindate, unate, or pure unate. It does this all using logic on bit masks of the columns to 
#   minimize the amound of scaning the cover it does. One example is that finds all pure unate columns before figuring out if they are 
#   positive or negative pure unate by ANDing the pure unate list with the has 0 and has 1 lists
#4. If all columns are unate and there isn't a universal cube (which at this point we know there isn't) the cover is not a tautology
#   In this stage the bitmaps for unate and binate columns are converted into a list of column indexes
#   If the length of the binate list is 0, all the columns are unate
#If it didn't match any special case, move onto General Unate Reduction (GUR):
#   This organizes the columns such that unate are on the left, and any universal cubes within the unate sub-cover are on the bottom
#   The bottom right corner of this cover forms the sub-cover F2 which indicates the tautology of the whole cover
#   One last special case that is checked here is if the unate covers don't have any universal cubes in their sub-cover
#   If this is the case, the cover is not a tautology.
#   The witness is all zeros with anything from the sub cover witness added in
#If all else fails, the last option is a binate split
#   This is where the cover is split in two via cofactoring with respect to the "most binate" column
#   The goal is to make two even halves of the parent cover that are each as small as possible
#   An efficient shortcut to this is simply picking whichever binate column has the least dahses since a dash would create a cube in both cofactors
#   If either cofactor is not a tautology, neither is the parent cover. If they both are, so is the parent cover
#   The witness is the same as the sub cover but the cofactored column is ste to 0 or 1 based on if it was the positive or negative cofactor that failed
...
def parse_espresso(filepath):                                                                       #Parses an Espresso file and returns the cover matrix, ilb, and ob
    ...
    for line in lines:                                                                              #Cycle through the lines
        ...
        if line[0] in '01-': cover.append([1 if c == '0' else 2 if c == '1' else 3 for c in line.split()[0]])   #If the line starts with 0 1 or - it's a cube. Split off the input part of the cube, and append it to the cover where '0' = 1, '1' = 2, '-' = 3
    ...
    cover = np.array(cover, dtype=np.uint8)                                                         #Convert to an np array
...
def is_tautology(cover, vars, depth, tracker):                                                      #Provide the cover, the variable names, the current recursion depth, and the stat tracker object to determine tautology
    ...
    if cover.size == 0:                                                                             #If it's an empty cover
    ...
    if 3 not in cover:                                                                              #If there are no '-' (3) in the cover                                                                          
        unique_cubes = frozenset(tuple(c) for c in cover)                                           #Convert the cover into a set of cubes
        ...
        for i in range(2**n):                                                                       #If the cover isn't a tautology, cycle through all possible cubes to find the missing minterm (witness)
            c_tuple = tuple(2 if ((i >> (n - 1 - j)) & 1) else 1 for j in range(n))                 #Build a tuple for the possible cube
            if c_tuple not in unique_cubes:                                                         #See if it's not in the set of unique cubes (one eventually won't be)
                wit = {vars[j]: 1 if c_tuple[j] == 2 else 0 for j in range(n)}                      #Reorganize the tuple to be in a dictionary object of all the variables
    ...
    if any((cover == 3).all(axis = 1)):                                                             #If there are any rows that are all '-' (3)
    ...
    has_0 = (cover == 1).any(axis=0)                                                                #Make an array of bools indicating if each column has a '0' (1)
    has_1 = (cover == 2).any(axis = 0)                                                              #Make an array of bools indicating if each column has a '1' (2)
    has_dashes = (cover == 3).any(axis = 0)                                                         #Make an array of bools indicating if each column has a '-' (3)
    binate_cols_mask = has_0 & has_1                                                                #Make an array of bools indicating if each column is binate
    unate_cols_mask = ~binate_cols_mask                                                             #Make an array of bools indicating if each column is unate
    pure_unate = ~(binate_cols_mask | has_dashes)                                                   #Make an array of bools indicating if each column is pure unate   
    if any(pure_unate):                                                                             #Check if any pure unate columns exist before figuring out if they are positive or negative
        all_1s = pure_unate & has_1                                                                 #Make an array of bools indicating if each column is all '1' (2)
        if any(all_1s):                                                                             #If any column is all '1' (2)
            tracker.all_1_or_0_count += 1                                                           #Add one to the tracker
            wit = {v: 0 for v in vars}                                                              #Create a witness dictionary with 0 for each variable name
            tracker.find_unate_binate_time += time.perf_counter() - t                               #Add the time it took to figure it out to the tracker
            return False, wit                                                                       #Return that the cover is not a tautology 
        all_0s = pure_unate & has_0                                                                 #Make an array of bools indicating if each column is all '0' (1)
        if any(all_0s):                                                                             #If any column is all '0' (1)
            tracker.all_1_or_0_count += 1                                                           #Add it to the tracker
            wit = {v: 1 for v in vars}                                                              #Create a witness dictionary with 1 for each variable name
            tracker.find_unate_binate_time += time.perf_counter() - t                               #Add the time it took to figure it out to the tracker
            return False, wit                                                                       #Return that the cover isn't a tautology
    ...
    unate_cols = np.where(unate_cols_mask)[0]                                                       #Conver the unate bool array into a list of column indexes
    binate_cols = np.where(binate_cols_mask)[0]                                                     #Conver the binate bool array into a list of column indexes
    if len(binate_cols) == 0:                                                                       #See if all the columns are unate (no binate ones)
        ...
        for i in range(n):                                                                          #Cycle through all the columns
            wit[vars[i]] = 0 if has_1[i] else 1                                                     #If there's a '1' (2) in the column value set, that variable is a 0, otherwise it's a 1
    ...
    if len(unate_cols) > 0:                                                                         #If there are unate columns                                             
        reduced_cover = cover[(cover[:, unate_cols] == 3).all(axis = 1)][:, binate_cols]            #Make a list of cubes that's unate variables are all '-' (3) and create a sub cover of just binate variables of those cubes (F2)
        if len(reduced_cover) == 0:                                                                 #If there are no cubes in this sub-cover, there are no universal cubes in the unate sub-cover
            ...
            for i in unate_cols:                                                                    #Go through all the unate variables
                wit[vars[i]] = 0 if has_1[i] else 1                                #If there's a '1' (2) set the variable in the witness dictionary to 0. If '1' (2) or '-' (3), set it to 1
            ...
            if res:                                                                                 #If the result is that it is a tautology, track time
                ...
                for i in range(n):                                                                  #Iterate through all the variables
                    if i in unate_cols:                                                             #For the unate columns
                        wit[vars[i]] = 0 if has_1[i] else 1                                         #If the set has a '1' (2) set that variable in the witness to a 0. 1 otherwise
    ...
    cur_find_unate_binate_time = tracker.find_unate_binate_time                                     #Find variable with least '-' (3). Not strictly most binate column
    ...
    dash_counts = np.sum(cover[:, binate_cols] == 3, axis=0)                                        #Count the number of dashes in the binate columns
    best_col = binate_cols[np.argmin(dash_counts)]                                                  #Get the column index of the column with the least dashes
    ...
    cofactor_1 = np.delete(cover[cover[:, best_col] != 1], best_col, axis=1)                        #Create the negative cofactor (x = 1)
    ...
    cofactor_0 = np.delete(cover[cover[:, best_col] != 2], best_col, axis=1)                        #Create the negative cofactor (x = 0)
...
if __name__ == "__main__":
    start_time = time.perf_counter()                                                                #Start timeer for file parsing
    input_file = sys.argv[1]                                                                        #Get input file from terminal
    cover, ilb, ob = parse_espresso(input_file)                                                     #Parse the file into a cover
    print(f"\nParsed {input_file} with {len(cover)} cubes in {(time.perf_counter() - start_time):.3f} sec")    #Print the time it took to parse
